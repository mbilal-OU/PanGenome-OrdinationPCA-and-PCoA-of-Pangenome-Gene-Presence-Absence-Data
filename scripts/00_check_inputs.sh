...
mkdir -p results/qc
{
  echo "PanGenome-Ordination input QC report"
  echo "============================"
  echo "Date: $(date)"
  echo
  echo "RTAB: $RTAB"
  ...
  echo "File sizes:"; ls -lh "$RTAB" "$CSV"; echo
  echo "Genome count from Rtab:"; head -n 1 "$RTAB" | tr '\t' '\n' | tail -n +2 | wc -l
  echo "Gene cluster count from Rtab:"; tail -n +2 "$RTAB" | wc -l
  echo
  echo "First 10 genome names:"; head -n 1 "$RTAB" | tr '\t' '\n' | tail -n +2 | head -n 10
} | tee "$REPORT"
